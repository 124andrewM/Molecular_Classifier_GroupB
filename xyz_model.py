# Purpose:
# This script is to load the xyz model and allow the user to make a prediction with it.
# It takes in one csv file as an argument, which contains the coordinates of the atoms in a molecule.
...
# to process one csv file as a dataset.
...
def add_energy_gap(file_path):
    gap = homo_lumo_energy(file_path)
    if DEBUG: print(f"Energy Gap calaculated as: {gap}")
...
# Parsing in the CSV file
def parse_comma_files(file_path):
  atoms, coords = [], []
  with open(file_path, 'r') as file:
    energy_gap = float(file.readline().split(',')[1])
    if DEBUG: print(f"Extracted energy gap from file as: {energy_gap}")
    for num, line in enumerate(file, start=2):
      single_atom = line.strip().split(',')
      if DEBUG: print(f"Extracted Atom from file: {single_atom}")
      if len(single_atom) == 4:
        atoms.append(single_atom[0])
        coords.append([float(val) for val in single_atom[1:]])
      else:
          print(f"Line: {num} did not contain the expected number of values, aborting script.")
          sys.exit(1)
    if (len(atoms) == 0):
        print("Did not find any atoms in the file, please check the formatting, aborting script.")
        sys.exit(1)
...
if __name__=='__main__':
   ...
   # Parse the arguments
    if len(sys.argv) != 2:
        print("Usage: python xyz-model.py <input_file.csv>")
    ...
    # Attempt to add the energy_gap
    # Will return an error if it already exists, we ignore this.
    try:
        add_energy_gap(file_path)
    except IndexError as e:
        print(f"Skipped adding the energy gap to the file, it may already be present, enable debug for more info.")
    except Exception as e:
        print(f"add_energy_gap() error: {e}")
